...
public class TaskExamServiceImpl extends BaseServiceImpl<TaskExam> implements TaskExamService {
    ...
    public void edit(TaskRequestVM model, User user) {
        ActionEnum actionEnum = (model.getId() == null) ? ActionEnum.ADD : ActionEnum.UPDATE; //判断是新增或者修改
        TaskExam taskExam = null; //任务列表字段
        if (actionEnum == ActionEnum.ADD) {  //如果是新增
            //使用modelMapper设置请求过来的数据
            Date now = new Date();
            ...
            taskExam.setCreateTime(now); //创建时间
            ...
            taskExamMapper.insertSelective(taskExam); //添加
